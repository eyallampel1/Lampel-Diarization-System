function [RDER] = SegScore2S(HypDir, RefDir, Collar, Detailed, ...
    Save, MaxConv)
...
for i = 3 : size(HypFiles,1)
    ...
    % Calculate error
    Err = find(Ref~=Hyp);
    DER(i-2) = length(Err)/length(Ref)*100;
    NSAS(i-2) = sum(((Ref(Err) == 0) & (Hyp(Err) == 1)) | ...
        ((Ref(Err) == 0) & (Hyp(Err) == 2)))/length(Ref)*100;
    SANS(i-2) = sum(((Ref(Err) == 1) & (Hyp(Err) == 0)) | ...
        ((Ref(Err) == 2) & (Hyp(Err) == 0)))/length(Ref)*100;
    SMIX(i-2) = sum(((Ref(Err) == 1) & (Hyp(Err) == 2)) | ...
        ((Ref(Err) == 2) & (Hyp(Err) == 1)) )/length(Ref)*100;
    SOVS(i-2) = sum((Ref(Err) == 3) & (Hyp(Err) ~= 3))/length(Ref)*100;
