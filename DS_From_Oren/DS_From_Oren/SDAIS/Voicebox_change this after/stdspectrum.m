...
%      Version: $Id: stdspectrum.m,v 1.5 2010/05/10 15:08:07 dmb Exp $
%
...
if isempty(spz)
    ...
    spz={[1 0];
        ...
        [1.1056962003903e+024 1 0 -25903.701047817 -23615.535213635+36379.908937329i -62675.170058468 -18743.746690721+62460.156452506i];
    ...
    if ixs==1
        ...
        for mm=1:length(sa)     % maximum number of poles
            for nn=1:mm         % number of zeros is always less than number of poles
                ifb=bz;        % initialize to best so far
                ifa=az;
                hn=hj;
                j=0;
